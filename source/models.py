...
class small_UNet(nn.Module):
    ...
    def __init__(self, input_channel=1, channels=[32, 64, 128, 256], embed_dim=256):
        ...
        self.conv5 = nn.Conv2d(channels[3], 1, 2, 1, 0, bias=False)
    
        self.output = Dense(channels[3]*4, 1)
        # The swish activation function
        self.act = nn.SiLU()
        #self.marginal_prob_std = marginal_prob_std
  
    def forward(self, x, t=None, linear_output=True): 
        if t != None:
        ...
        h4 = self.act(h4)
        if linear_output:
            h5 = self.output(h4.view(x.shape[0], -1))
        else:
            h5 = self.conv5(h4)
        return h5.view(-1, 1)
